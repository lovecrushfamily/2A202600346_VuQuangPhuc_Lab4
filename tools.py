...
def search_flights(origin: str, destination: str) -> str:
    ...
    try:
        ...
        if flights:
            ...
            return "\n".join(
                [f"Các chuyến bay từ {key[0]} đến {key[1]}:"] + formatted
            )
        ...
        if reverse_flights:
            return (
                ...
                f"Tuy nhiên có chiều ngược lại từ {destination} đến {origin}. "
                f"Hãy kiểm tra lại hướng di chuyển."
            )
...
def search_hotels(city: str, max_price_per_night: int = 9_999_999) -> str:
    ...
    try:
        ...
        formatted = [
            f"- {hotel['name']} | {hotel['stars']} sao | {_format_vnd(hotel['price_per_night'])}/đêm | "
            f"{hotel['area']} | rating {hotel['rating']}"
            for hotel in filtered_hotels
        ]
        return "\n".join(
            [
                f"Các khách sạn phù hợp tại {city} (tối đa {_format_vnd(max_price_per_night)}/đêm):"
            ]
            + formatted
        )
    except Exception as exc:
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for raw_item in expenses.split(","):
            ...
            if ":" not in item:
                raise ValueError(
                    f"Khoản chi '{item}' không đúng định dạng tên_khoản:số_tiền."
                )
        ...
        if remaining >= 0:
            ...
            lines.append(
                f"Vượt ngân sách {-remaining:,}".replace(",", ".")
                + "đ! Cần điều chỉnh."
            )
